...
class OrderExecutor:
    ...
    def place_order(self, contract, side: str, qty: int, limit_price: float) -> Optional[Any]:
        ...
        order = LimitOrder(action=action, totalQuantity=qty, lmtPrice=limit_price, tif="DAY", outsideRth=False)
    ...
    def log_trade(
        ...
        if hasattr(contract, "lastTradeDateOrContractMonth"):
            ...
            structure = (
                f"Long {'Call' if contract.right == 'C' else 'Put'}"
                if side == "BUY"
...
def main():
    ...
    try:
        ...
        if result["status"] == "filled" and not args.no_log:
            executor.log_trade(result, contract, args.side, limit_price, thesis=args.thesis, notes=args.notes)
