...
def fetch_repair_market_data() -> Tuple[Dict[str, np.ndarray], List[str]]:
    ...
    if len(common_dates) < MIN_REPAIR_BARS:
        raise RuntimeError(f"Only {len(common_dates)} common repair dates (need {MIN_REPAIR_BARS})")
...
def build_repaired_cri_payload(
    ...
    for index in range(max(0, len(common_dates) - 20), len(common_dates)):
        ...
        day_cor1m = (
            current_cor1m
            if day_date == last_date and current_cor1m is not None
            else as_number(cor1m_by_date.get(day_date))
        )
...
def repair_cache(target_date: Optional[str] = None) -> Dict[str, Any]:
    ...
    if target_date and repaired.get("date") != target_date:
        raise RuntimeError(f"Repair produced {repaired.get('date')} but target date is {target_date}")
    return repaired
