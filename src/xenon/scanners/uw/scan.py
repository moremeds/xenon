...
from xenon.scanners.uw.context.pcr_sentiment import flag as pcr_flag
from xenon.scanners.uw.models import ContextFlag, ScanCandidate, SignalHit
from xenon.scanners.uw.ranking import build_candidate, rank_candidates
from xenon.scanners.uw.signals.dark_pool_accumulation import detect as dp_detect
from xenon.scanners.uw.signals.deep_conviction_flow import detect as dcf_detect
from xenon.scanners.uw.signals.earnings_iv_crush import detect as eic_detect
from xenon.scanners.uw.signals.gex_pinning import detect as gp_detect
...
def scan_universe(cfg: ScanConfig, *, client) -> dict:
    ...
    def _process(ticker: str) -> Optional[ScanCandidate]:
        ...
        gates_result = {
            "earnings": "pass" if earnings_gate(earnings_within_14d=td.earnings_within_14d) else "block",
            "liquidity": "pass" if liquidity_gate(option_volume=option_volume) else "block",
...
def _candidate_to_dict(c: ScanCandidate) -> dict:
    return {
        ...
        "context_flags": [{"layer": f.layer, "label": f.label, "value": f.value} for f in c.context_flags],
        "gates": c.gates,
    }
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    p.add_argument("--min-confluence", type=int, default=0, help="Require N independent signals (2 = Type F only)")
    p.add_argument("--analyze-top", type=int, default=0, help="Chain to uw_analyze for top N candidates")
    ...
    if args.watchlist:
        ...
        print(
            "ERROR: specify tickers or --watchlist. Market-wide mode is "
    ...
    if args.json:
        ...
        print(
            f"Scan complete: {result['candidates_with_hits']} candidates "
