...
def compute_source_delta(levels: Dict[str, Any], mq: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
...
def bucket_profile(
    ...
    buckets: Dict[float, Dict[str, float]] = defaultdict(lambda: {"call_gex": 0.0, "put_gex": 0.0, "net_gex": 0.0})
...
def find_key_levels(profile: List[Dict[str, Any]], spot: float) -> Dict[str, Optional[Dict[str, Any]]]:
...
def compute_expected_range(spot: float, atm_iv: Optional[float]) -> Dict[str, Any]:
...
def compute_directional_bias(
    ...
    magnet_above = levels.get("max_magnet") is not None and levels["max_magnet"]["strike"] > spot
...
def build_gex_output(
    ...
    iv_data: Dict[str, Any] = {
        ...
        "source": (
            "both"
            if (atm_iv and mq and mq.get("iv30d"))
            else "uw"
            if atm_iv
            else "mq"
            if (mq and mq.get("iv30d"))
            else None
        ),
    }
...
def main():
    ...
    try:
        ...
        if atm_iv:
            print(
                f"  IV 30D: {atm_iv * 100:.1f}%  IV Rank: {iv_rank_val:.1f}%"
                if iv_rank_val
                else f"  IV 30D: {atm_iv * 100:.1f}%",
                file=sys.stderr,
            )
    ...
    if not args.no_mq:
        ...
        if mq_data:
            ...
            print(
                f"  MQ HVL={hvl}  CallResist={cr}  PutSupport={ps}" + (f"  IV30D={mq_iv:.1f}%" if mq_iv else ""),
                file=sys.stderr,
            )
        else:
...
def _print_summary(result: Dict[str, Any]) -> None:
    ...
    for name in ("max_magnet", "second_magnet", "max_accelerator", "put_wall"):
        ...
        if lvl:
            ...
            print(
                f"  {label:14s}: {lvl['strike']:,.0f} (gamma={lvl['gamma']:+,.2f}, {lvl['distance_pct']:+.1f}%)",
                file=sys.stderr,
            )
