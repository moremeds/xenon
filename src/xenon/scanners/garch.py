...
def analyze_pair(a: str, b: str, vol_data: Dict[str, TickerVol], vol_driver: str = "") -> PairAnalysis:
...
def resolve_inputs(tickers: List[str], preset_name: Optional[str]) -> Tuple[List[str], List[List[str]], str, str]:
...
def generate_html(
    ...
    nearest_label = (
        f"{nearest.lagger} (+{nearest.lagger_hv_iv_gap:.0f})" if nearest and nearest.lagger_hv_iv_gap > 0 else "—"
    )
...
<thead><tr>
  ...
  <th class="text-center">Status</th>
    ...
    for ticker in sorted(all_vol.keys()):
        ...
        if tv.error:
            body_parts.append(
                f'<tr><td><strong>{ticker}</strong></td><td colspan="8" class="text-muted">{tv.error}</td></tr>'
            )
            continue
        ratio = tv.iv_hv60
        gap = tv.hv20_minus_iv
    ...
    return {
        ...
        "tickers": {
            t: {
                ...
                "hv60": v.hv60,
                "hv252": v.hv252,
                "leap_atm_iv": v.leap_atm_iv,
                "iv_rank": v.iv_rank,
                "iv_hv60": round(v.iv_hv60, 3),
                "hv20_minus_iv": round(v.hv20_minus_iv, 1),
                "has_leaps": v.has_leaps,
                "leap_count": v.leap_count,
            }
...
""",
    ...
    if args.json:
        ...
        out_path = (
            Path(args.output)
            if args.output
            else _PROJECT_DIR / f"reports/garch-convergence-{preset_slug}-{date_str}.html"
        )
