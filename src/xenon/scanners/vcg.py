        ...
        with UWClient() as uw:
            for ticker in fetchable:
                try:
                    ...
                    bars = data.get("data", [])
                    if bars:
                        parsed = [(b["date"], float(b["close"])) for b in bars if b.get("close") is not None]
                        if parsed:
...
def _fetch_yahoo(ticker: str, days: int = 400) -> List[Tuple[str, float]]:
    ...
    url = f"https://query1.finance.yahoo.com/v8/finance/chart/{yahoo_sym}?period1={start}&period2={end}&interval=1d"
...
def evaluate_signal(
    ...
    ro = bool(not np.isnan(vcg_val) and vix > vix_floor and vcg_val > vcg_trigger and sign_ok)

    # Watch state: Elevated Divergence Risk (EDR)
    edr = bool(not np.isnan(vcg_val) and vix > VIX_EDR and vcg_val > VCG_TRIGGER and sign_ok)
    ...
    total_component = (
        abs(vvix_component) + abs(vix_component) if (abs(vvix_component) + abs(vix_component)) > 1e-12 else 1.0
    )
...
def print_summary(
    ...
    if signal["ro"] == 1:
        ...
        print(
            f"\n  EDR active — elevated divergence risk. Monitor for VCG escalation above {VCG_RO_TRIGGER}.",
            file=sys.stderr,
        )
...
def build_json_output(
    ...
    for i in range(max(0, n - 20), n):
        ...
        sign_ok_i = not np.isnan(beta1_i) and beta1_i <= 0 and not np.isnan(beta2_i) and beta2_i <= 0
        ro_i = bool(not np.isnan(vcg_i) and vix_i > vix_floor and vcg_i > vcg_trigger and sign_ok_i)
        edr_i = bool(not np.isnan(vcg_i) and vix_i > VIX_EDR and vcg_i > VCG_TRIGGER and sign_ok_i)
...
<header class="header">
  ...
  <div class="header-actions">
    ...
    vcg_cls = (
        "text-negative"
        if signal["vcg"] is not None and signal["vcg"] > VCG_TRIGGER
        else "text-positive"
        if signal["vcg"] is not None and signal["vcg"] < -VCG_TRIGGER
        else ""
    )
...
<thead><tr><th>Condition</th><th class="text-center">Required</th><th class="text-center">Actual</th>"""
    ...
    for label, req, actual, ro_pass, edr_pass in signal_rows:
        ro_icon = (
            '<span class="text-positive">&#10003;</span>' if ro_pass else '<span class="text-negative">&#10007;</span>'
        )
        edr_icon = (
            '<span class="text-positive">&#10003;</span>' if edr_pass else '<span class="text-negative">&#10007;</span>'
        )
        body_parts.append(
    ...
    sign_icon = (
        '<span class="text-positive">OK</span>'
        if signal["sign_ok"]
        else '<span class="text-negative">SUPPRESSED</span>'
    )
...
""",
    ...
    if args.backtest:
        ...
        print(
            f"\n  Backtest ({args.days}d): {ro_days} RO signals, {edr_days} EDR days, {bounce_days} bounce days",
            file=sys.stderr,
        )
    ...
    if args.json:
        result = build_json_output(
            signal, model, common_dates, proxy, market_open, bt, vix_floor=args.vix_floor, vcg_trigger=args.vcg_trigger
        )
