...
def fetch_cor1m_current_quote() -> Optional[float]:
    ...
    if ib_quote is not None and cboe_quote is not None and selected == cboe_quote and abs(ib_quote - cboe_quote) > 1.0:
        ...
        ib_quote is not None
        and yahoo_quote is not None
        and selected == yahoo_quote
        and abs(ib_quote - yahoo_quote) > 1.0
    ):
        print(
...
def append_post_close_snapshot(
    ...
    validated_snapshot = {ticker: _valid_quote_value(closing_snapshot.get(ticker)) for ticker in required}
    ...
    extended = {ticker: np.append(values, float(validated_snapshot[ticker])) for ticker, values in aligned.items()}
...
def run_analysis(
    ...
    menthorq_cta = None
    try:
        from xenon.fetchers.fetch_menthorq_cta import find_by_underlying, load_menthorq_cache
...
<div class="grid-2">
  ...
  <div class="panel">
    ...
    if menthorq and menthorq.get("tables"):
        body_parts.append(
            f"""<div class="section-header">MenthorQ CTA Positioning <span style="font-size:10px;color:var(--text-muted);font-weight:400">(institutional data — {menthorq.get("date", "---")})</span></div>"""
        )
