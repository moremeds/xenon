...
from xenon.clients.uw_client import UWAPIError, UWClient, UWRateLimitError
...
def get_existing_tickers() -> set:
    ...
    if WATCHLIST.exists():
        with open(WATCHLIST) as f:
            data = json.load(f)
            tickers.update(
                t.get("ticker") or t.get("symbol", "") for t in data.get("tickers", []) if isinstance(t, dict)
            )
    if PORTFOLIO.exists():
...
def calculate_score(
    dp_strength: float, dp_sustained: int, has_confluence: bool, vol_oi_ratio: float, sweep_count: int, alert_count: int
) -> dict:
    """
...
def resolve_tickers(args_tickers: list) -> list:
    ...
    """
    from xenon.utils.presets import PRESETS_DIR, load_preset
...
def _build_candidate(ticker: str, flow_data: dict, dp: dict) -> dict:
    ...
    # Check confluence
    has_confluence = (options_bias == "BULLISH" and dp_agg["direction"] == "ACCUMULATION") or (
...
def discover_targeted(
    tickers: list, dp_days: int = 3, min_premium: int = 50000, top: int = 20, max_workers: int = 10
) -> dict:
    """
    ...
    with UWClient() as client:
        ...
        def _process_targeted(ticker):
            ...
            flow_data = _aggregate_alerts(alerts).get(
                ticker,
                {
                    "alerts": 0,
                    "total_premium": 0,
...
def main():
    ...
    p.add_argument(
        "tickers", nargs="*", help="Tickers and/or preset names (e.g. AAPL ndx100). Omit for market-wide scan."
    )
    p.add_argument(
        "--min-premium",
        type=int,
    ...
    p.add_argument(
        "--min-alerts", type=int, default=1, help="Minimum alerts per ticker in market-wide mode (default 1)"
    )
    p.add_argument("--dp-days", type=int, default=3, help="Days of dark pool data to check (default 3)")
    p.add_argument("--top", type=int, default=20, help="Number of top candidates to return (default 20)")
    p.add_argument(
        "--include-indices", action="store_true", help="Include index options (SPX, etc) in market-wide mode"
    )
    args = p.parse_args()
