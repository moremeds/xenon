...
def scan_etf(
    client: IBClient, ticker: str, sector: str, target_years: list, target_deltas: list, min_gap: float
) -> Optional[ScanResult]:
    ...
    for expiry in expirations:
        ...
        for target_delta, opt in delta_matches.items():
            ...
            print(
                f"    {delta_label} ${opt['strike']}: IV={opt['iv']:.1f}% | "
    ...
    return ScanResult(volatility=vol_data, options=all_options, mispriced_count=len(mispriced), best_opportunity=best)
...
def generate_report(results: list, min_gap: float, target_years: list = None) -> str:
    ...
    results_sorted = sorted(
        [r for r in results if r.best_opportunity], key=lambda x: x.best_opportunity.mispricing_score, reverse=True
    )
...
Available presets: sectors, mag7, semis, financials, energy, china, emerging
    ...
    parser.add_argument("--preset", type=str, choices=list(PRESETS.keys()), help="Use a preset ticker group")
    parser.add_argument("--portfolio", action="store_true", help="Scan tickers from portfolio.json")
    ...
    parser.add_argument(
        "--min-gap", type=float, default=MIN_IV_GAP, help=f"Minimum HV-IV gap to flag (default: {MIN_IV_GAP}%%)"
    )
    parser.add_argument(
        "--deltas", type=float, nargs="+", default=TARGET_DELTAS, help=f"Target deltas (default: {TARGET_DELTAS})"
    )
    ...
    # Output
    parser.add_argument("--output", type=str, default="reports/leap-iv-scan.html", help="Output HTML report path")
    ...
    try:
        for ticker, sector in tickers_to_scan.items():
            try:
                result = scan_etf(client, ticker, sector, target_years, target_deltas, args.min_gap)
                if result:
        ...
        if total_mispriced > 0:
            ...
            for vol, opt in all_mispriced[:5]:
                print(
                    f"   {opt.ticker} {opt.expiry} ${opt.strike} {int(opt.delta * 100)}Δ: "
