...
Usage:
    from xenon.utils.market_hours import is_market_open, get_market_status
"""
...
def get_market_status(dt: datetime = None) -> Tuple[bool, str]:
    ...
    if is_open:
        ...
        if weekday >= 5:
            ...
            mins_to_open = int(
                (datetime.combine(dt.date(), MARKET_OPEN) - datetime.combine(dt.date(), current_time)).total_seconds()
                / 60
            )
...
def main() -> None:
    """CLI entry point: print current market status."""
