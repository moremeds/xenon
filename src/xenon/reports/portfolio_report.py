...
def fetch_ib_data(ib) -> Tuple[List[Dict], Dict[str, str]]:
    ...
    for pos in raw_positions:
        ...
        last = (
            float(t.last) if t and t.last and t.last > 0 else (float(t.close) if t and t.close and t.close > 0 else -1)
        )
        ...
        pnl = (
            ((entry_per - mid) * multiplier * abs(qty) if qty < 0 else (mid - entry_per) * multiplier * qty)
            if mid > 0
            else 0
        )
    ...
    def _fetch_one(ticker: str) -> Tuple[str, Dict]:
        try:
            ...
            for dt in days_sorted:
                ...
                total_buy += bv
                total_sell += sv
                direction = "ACCUMULATION" if ratio >= 0.7 else "DISTRIBUTION" if ratio <= 0.3 else "NEUTRAL"
            ...
            agg_dir = "ACCUMULATION" if agg >= 0.7 else "DISTRIBUTION" if agg <= 0.3 else "NEUTRAL"
...
def _multi_leg(sym, exp, legs):
    ...
    if stk_legs and short_calls and not long_calls and not puts:
        ...
        if long_shares >= short_contracts * 100:
            ...
            return {
                ...
                "mkt_val": sum(l["mkt_val"] for l in legs),
                "pnl": total_pnl,
                "pnl_pct": pnl_pct,
                "dte": sc.get("dte", dte),
                "expiry": opt_expiry if opt_expiry and opt_expiry != "stock" else exp,
                "risk": risk,
            }
    ...
    return {
        ...
        "pnl_pct": pnl_pct,
        "dte": dte,
        "expiry": exp if exp != "stock" else None,
        "risk": risk,
    }
...
def load_trade_log() -> Dict[str, Dict]:
    ...
    with open(TRADE_LOG_PATH) as f:
        data = json.load(f)
    return {t["ticker"]: t for t in data.get("trades", []) if t.get("decision") == "EXECUTED" and "close_date" not in t}
...
def status_pill(pnl_pct, dte):
    if dte is not None and dte <= 0:
        return '<span class="pill pill-negative">EXPIRING TODAY</span>'
...
def build_sparkline(days: List[Dict]) -> str:
    ...
    for d in days:  # oldest → newest
        ...
        if ratio >= 0.7:
            bar_cls = "accumulation"
        elif ratio <= 0.3:
            bar_cls = "distribution"
        else:
            bar_cls = "neutral"
        ...
        if d.get("is_today"):
            tooltip += " (TODAY)"
        html += f'<div class="spark-bar {bar_cls}{today_cls}" title="{tooltip}" style="height:{height}px"></div>'
    html += "</div>"
...
def build_today_cell(today_data: Optional[Dict]) -> str:
    ...
    cls = (
        "accumulation" if direction == "ACCUMULATION" else "distribution" if direction == "DISTRIBUTION" else "neutral"
    )
    return f'<span class="flow-dir {cls}">{ratio:.0%}</span><span class="today-tag">LIVE</span>'
...
def build_metrics_html(account: Dict, grouped: List[Dict], trade_log: Dict) -> str:
    ...
    kelly_sizes = [
        t.get("kelly_calculation", {}).get("actual_size_pct", 0)
        for t in trade_log.values()
        if t.get("kelly_calculation", {}).get("actual_size_pct")
    ]
...
def build_attention_html(grouped: List[Dict]) -> str:
    ...
    if expiring:
        li = "".join(
            f"<li><strong>{g['symbol']}</strong> — {g['structure']} | {g['dte']} DTE | {fp(g['pnl_pct'])} ({fc(g['pnl'], True)}) | {g['risk'].upper()}</li>"
            for g in expiring
        )
        parts.append(
            f'<div class="callout negative"><div class="callout-title">🔴 Expiring This Week</div><ul>{li}</ul></div>'
        )
    if at_stop:
        li = "".join(
            f"<li><strong>{g['symbol']}</strong> — {g['structure']} | {fp(g['pnl_pct'])} ({fc(g['pnl'], True)})</li>"
            for g in at_stop
        )
        parts.append(
            f'<div class="callout warning"><div class="callout-title">🟡 At or Below Stop (≤-50%)</div><ul>{li}</ul></div>'
        )
    if winners:
        li = "".join(
            f"<li><strong>{g['symbol']}</strong> — {g['structure']} | {fp(g['pnl_pct'])} ({fc(g['pnl'], True)}) | {g['dte'] or 'No'} DTE</li>"
            for g in winners
        )
        parts.append(
            f'<div class="callout positive"><div class="callout-title">🟢 Big Winners — Consider Profit Taking</div><ul>{li}</ul></div>'
        )
    if undefined:
        li = "".join(
            f"<li><strong>{g['symbol']}</strong> — {g['structure']} | {g['dte'] or 'N/A'} DTE</li>" for g in undefined
        )
        parts.append(
            f'<div class="callout negative"><div class="callout-title">⛔ Undefined Risk Positions (Rule Violation)</div><ul>{li}</ul></div>'
        )
    if not parts:
        parts.append(
            '<div class="callout positive"><div class="callout-title">✓ No Immediate Actions Required</div><p>All positions within normal parameters.</p></div>'
        )
    return "\n".join(parts)
...
def build_flow_html(grouped: List[Dict], flow_data: Dict) -> str:
    ...
    for sym in sorted(set(g["symbol"] for g in grouped)):
        ...
        dir_cls = (
            "accumulation"
            if flow["direction"] == "ACCUMULATION"
            else "distribution"
            if flow["direction"] == "DISTRIBUTION"
            else "neutral"
        )
        rows.append(f"""
        <tr>
...
def generate_report(grouped, account, flow_data, trade_log, timestamp, market_open) -> str:
    ...
    if market_open:
        freshness_cls = ""
        freshness_txt = (
            f"📊 {timestamp} · Market OPEN · All prices and flow data include <strong>today ({TODAY_STR})</strong>"
        )
    else:
        freshness_cls = "stale"
