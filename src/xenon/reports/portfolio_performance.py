    ...
    for row in df.to_dict(orient="records"):
        ...
        fills.append(
            TradeFill(
                ...
                multiplier=safe_float(row.get("multiplier"), default=100.0 if asset == "OPT" else 1.0)
                or (100.0 if asset == "OPT" else 1.0),
                security_type=asset,
                symbol=symbol,
                option_id=option_id,
                expiry=expiry,
...
def fetch_stock_history(
    symbol: str, start_date: str, end_date: str, ib_client: Optional[IBClient], uw_client: Optional[UWClient]
) -> Dict[str, float]:
...
def fetch_option_history(
    option_id: str, start_date: str, end_date: str, uw_client: Optional[UWClient]
) -> Dict[str, float]:
...
def align_mark_series(
    calendar: Iterable[str], raw_marks: Mapping[str, float], expiry: Optional[str] = None
) -> pd.Series:
            ...
            if abs(computed - actual) > 0.01:
                ...
                trades.append(
                    TradeFill(
                        ...
                        else re.match(r"^([A-Z.]+)", key).group(1)
...
def compute_performance_metrics(equity: pd.Series, benchmark: pd.Series) -> Dict[str, float]:
    ...
    annualized_return = (
        float((1.0 + total_return) ** (TRADING_DAYS / max(len(portfolio_returns), 1)) - 1.0) if len(equity) > 1 else 0.0
    )

    volatility = float(portfolio_returns.std(ddof=1) * math.sqrt(TRADING_DAYS)) if len(portfolio_returns) > 1 else 0.0
    downside_rms = (
        float(np.sqrt(np.mean(np.square(np.minimum(portfolio_returns.values, 0.0)))))
        if len(portfolio_returns) > 0
        else 0.0
    )
    downside_deviation = downside_rms * math.sqrt(TRADING_DAYS)
    sharpe_ratio = (
        float((portfolio_returns.mean() / portfolio_returns.std(ddof=1)) * math.sqrt(TRADING_DAYS))
        if len(portfolio_returns) > 1 and portfolio_returns.std(ddof=1) > 0
        else 0.0
    )
    sortino_ratio = (
        float((portfolio_returns.mean() / downside_rms) * math.sqrt(TRADING_DAYS)) if downside_rms > 0 else 0.0
    )
    ...
    if len(portfolio_returns) > 1 and len(benchmark_returns) > 1:
        ...
        correlation = (
            float(np.corrcoef(portfolio_returns.values, benchmark_returns.values)[0, 1])
        ...
        information_ratio = (
            float((active_returns.mean() / active_vol) * math.sqrt(TRADING_DAYS)) if active_vol > 0 else 0.0
        )
...
def _fetch_stock_history_ib_only(
    ...
    try:
        ...
        parsed = {str(bar.date)[:10]: float(bar.close) for bar in bars if start <= str(bar.date)[:10] <= end}
        if parsed:
...
def _fetch_stock_history_fallback(symbol: str, start: str, end: str) -> Tuple[str, Dict[str, float], str]:
...
def _fetch_option_history_safe(option_id: str, start: str, end: str) -> Tuple[str, Dict[str, float], Optional[str]]:
...
def build_payload(benchmark_symbol: str = "SPY") -> dict:
    ...
    metrics = compute_performance_metrics(curve["equity"], benchmark_series)
    benchmark_total_return = (
        float((benchmark_series.iloc[-1] / benchmark_series.iloc[0]) - 1.0) if len(benchmark_series) > 1 else 0.0
    )
        ...
        series.append(
            {
                ...
                "equity": round(float(curve.loc[dt, "equity"]), 4),
                "daily_return": None
                if pd.isna(curve.loc[dt, "daily_return"])
                else round(float(curve.loc[dt, "daily_return"]), 8),
                "drawdown": round(float(curve.loc[dt, "drawdown"]), 8),
    ...
    return {
        ...
        "summary": {
            ...
            **{
                key: round(float(value), 8) if isinstance(value, (float, np.floating)) else value
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.json:
        ...
        print(
            f"Sharpe: {summary['sharpe_ratio']:.2f} | Sortino: {summary['sortino_ratio']:.2f} | Max DD: {summary['max_drawdown'] * 100:.2f}%"
        )
        if payload["warnings"]:
            print("\nWarnings:")
