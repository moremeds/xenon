...
def fetch_chain(
    ...
    target_expiries = sorted(
        e for e in chain.expirations if min_dte <= (datetime.strptime(e, "%Y%m%d") - today).days <= max_dte
    )
...
def _build_trade_panel(
    combo: Dict, label: str, pill_class: str, pill_text: str, spot: float, panel_class: str = "panel"
) -> str:
...
def generate_report(
    ...
    net_label = (
        "CREDIT"
        if primary and primary["net"] >= 0
        else "COSTLESS"
        if primary and abs(primary["net"]) < 0.10
        else "SMALL DEBIT"
    )
    ...
    for d in daily:
        ...
        dir_class = (
            "text-positive"
            if direction_day == "ACCUMULATION"
            else "text-negative"
            if direction_day == "DISTRIBUTION"
            else "text-muted"
        )
    ...
    primary_html = _build_trade_panel(
        matrix["primary"], "⭐ Primary Recommendation", "pill-positive", "RECOMMENDED", spot, "panel panel-accent"
    )
    alternative_html = _build_trade_panel(
        matrix["alternative"], "Alternative — Different Expiry", "", "ALTERNATIVE", spot
    )
    ...
    for exp, combos in sorted(combos_by_exp.items()):
        ...
        for c in combos[:20]:  # Limit to top 20 per expiry
            is_primary = (
                matrix["primary"]
                and c["short_strike"] == matrix["primary"]["short_strike"]
                and c["long_strike"] == matrix["primary"]["long_strike"]
                and c["expiry"] == matrix["primary"]["expiry"]
            )
            is_alt = (
                matrix["alternative"]
                and c["short_strike"] == matrix["alternative"]["short_strike"]
                and c["long_strike"] == matrix["alternative"]["long_strike"]
                and c["expiry"] == matrix["alternative"]["expiry"]
            )
...
def main():
    parser = argparse.ArgumentParser(description="Risk Reversal Scanner — exploit IV skew for directional bets")
