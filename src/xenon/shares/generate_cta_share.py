...
def card1_squeeze(data: dict, ds: str) -> str:
    ...
    selling_bn = (
        data.get("cta_model", {}).get("est_selling_bn", 90.6) if isinstance(data.get("cta_model"), dict) else 90.6
    )
...
def card2_equity(data: dict, ds: str) -> str:
    ...
    for name, r in INDICES:
        ...
        if pctile <= 5:
            extreme_count += 1
        pctile_style = (
            "background:rgba(232,93,108,0.2);color:#E85D6C;font-weight:700;padding:1px 5px;border-radius:2px"
            if pctile <= 10
            else "color:#94a3b8"
        )
...
def card3_commodities(data: dict, ds: str) -> str:
    ...
    crowded = sorted(
        [r for r in comm if r["percentile_3m"] >= 80 and r["position_today"] > 0], key=lambda r: -r["percentile_3m"]
    )[:5]
...
def screenshot_card(html_path: str, png_path: str) -> bool:
    try:
        result = subprocess.run(
            ["agent-browser", "open", f"file://{html_path}"], capture_output=True, text=True, timeout=15
        )
        if result.returncode != 0:
            return False
        result2 = subprocess.run(
            ["agent-browser", "screenshot", ".card", png_path], capture_output=True, text=True, timeout=15
        )
...
<div class="layout">
  <div class="intro"><strong>CTA Report — X Share</strong><br>Tweet text + 4 infographic cards · {ds} · Analyzed by Xenon</div>
  <div class="panel">
...
def build_tweet(data: dict, ds: str) -> str:
    ...
    if spx_z <= -2.0:
        ...
        hook = f"📊 CTA positioning update ({month_day}): SPX at {spx_pos:+.2f}, z-score {spx_z:.2f}."
    ...
    if bonds_10y:
        ...
        if b_z <= -1.5:
            cross_asset_lines.append(f"> Bonds also max short: 10Y at {b_pos:+.2f} (z={b_z:.2f}) — full curve short")
    ...
    if gold:
        ...
        if abs(g_z) > 1.5:
            ...
            cross_asset_lines.append(f"> Gold CTAs {direction} at {g_pos:+.2f} (z={g_z:.2f})")
    ...
    if spx_z <= -1.5:
        ...
        conclusion = "No extreme positioning. CTAs are adjusting normally to the current vol regime."

    parts = [hook, "", thesis]
    if cross_asset:
        parts.extend(["", cross_asset])
    parts.extend(["", conclusion, "", "Analyzed by Xenon · "])
