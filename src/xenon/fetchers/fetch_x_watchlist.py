...
def fetch_tweets_agent_browser(account: str, hours: int = 24) -> List[Dict]:
    ...
    for line in lines:
        ...
        if tickers and len(line) > 20:
            tweets.append({"text": line, "timestamp": "recent", "tickers": tickers})
...
def main():
    ...
    parser.add_argument("--no-update", action="store_true", help="Don't update watchlist, just display results")
    ...
    if args.test:
        ...
        for text in sample_tweets:
            ...
            if tickers:
                tweets.append(
                    {"text": text, "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M"), "tickers": tickers}
                )
    else:
