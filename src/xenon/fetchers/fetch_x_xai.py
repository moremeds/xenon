...
def xai_search(account: str, days: int = 3) -> Dict:
    ...
    cmd = [
        ...
        "-H",
        f"Authorization: Bearer {XAI_API_KEY}",
        "-H",
        ...
        for content_item in content_list:
            if content_item.get("type") == "output_text":
                ...
                patterns = [
                    ...
                    re.compile(
                        r"\*\*\\\$([A-Z]{1,5})\*\*\s*\((Bullish|Bearish|Neutral)[^)]*\):\s*(.+?)(?=\n-|\n\n|$)",
                        re.IGNORECASE | re.DOTALL,
                    ),
                    # $TICKER: Sentiment - explanation or just $TICKER: Sentiment
                    re.compile(
                        r"[-•]\s*\\\$([A-Z]{1,5}):\s*(Bullish|Bearish|Neutral)(?:\s*[-–]\s*(.+?))?(?=\n[-•]|\n\n|$)",
                        re.IGNORECASE | re.DOTALL,
                    ),
                    # **$TICKER**: Sentiment
...
def update_watchlist(
    account: str, tweets: List[Dict], watchlist_path: str = "data/watchlist.json"
...
def main():
    ...
    parser.add_argument("--account", "-a", required=True, help="X account to scan (without @)")
    parser.add_argument("--days", "-d", type=int, default=3, help="Days to look back (default: 3)")
    parser.add_argument("--dry-run", action="store_true", help="Don't update watchlist, just display results")
    parser.add_argument("--json", action="store_true", help="Output raw JSON response")
    parser.add_argument("--timeout", "-t", type=int, default=180, help="API timeout in seconds (default: 180)")
