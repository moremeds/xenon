...
| `apex-refresh`                   | Apex R2 ETL entrypoint. Nightly GitHub Action (`.github/workflows/apex-data-refresh.yml`). Local dry-run: `python3.13 scripts/apex_refresh.py --mode full --dry-run --timeframes 1d,1h`. Writes OHLCV + TA-indicator parquets to R2 `apex-data` bucket. |
