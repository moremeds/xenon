...
from xenon.scanners.scanner import scan
...
class TestScannerParallelExecution:
    ...
    @patch("xenon.scanners.scanner.get_open_positions", return_value=set())
    @patch("xenon.scanners.scanner.WATCHLIST", new_callable=PropertyMock)
    def test_multiple_tickers_processed_concurrently(self, mock_wl_path, mock_positions, tmp_path):
        ...
        with patch("xenon.scanners.scanner.WATCHLIST", wl_file), \
             patch("xenon.scanners.scanner.fetch_flow_data", side_effect=slow_fetch):
            scan(top_n=50)
    ...
    def test_results_collected_from_all_tickers(self, mock_positions, tmp_path, capsys):
        ...
        with patch("xenon.scanners.scanner.WATCHLIST", wl_file), \
             patch("xenon.scanners.scanner.fetch_flow_data", return_value=_make_flow_data()):
            scan(top_n=50)
...
class TestScannerErrorHandling:
    ...
    def test_rate_limit_error_skips_ticker(self, mock_positions, tmp_path, capsys):
        ...
        with patch("xenon.scanners.scanner.WATCHLIST", wl_file), \
             patch("xenon.scanners.scanner.fetch_flow_data", side_effect=fetch_side_effect):
            scan(top_n=50)
...
class TestScannerMaxWorkers:
    ...
    def test_max_workers_parameter_passed_to_executor(self, mock_positions, tmp_path, capsys):
        ...
        with patch("xenon.scanners.scanner.WATCHLIST", wl_file), \
             patch("xenon.scanners.scanner.fetch_flow_data", return_value=_make_flow_data()), \
             patch("xenon.scanners.scanner.ThreadPoolExecutor", wraps=ThreadPoolExecutor) as mock_executor:
