...
class TestGetUwToken:
    def test_returns_token_when_set(self):
        from xenon.utils.uw_api import get_uw_token
...
class TestUwBaseUrl:
    def test_base_url_constant(self):
        from xenon.utils.uw_api import UW_BASE_URL

        assert UW_BASE_URL == "https://api.unusualwhales.com/api"


class TestUwApiGet:
    @patch("xenon.utils.uw_api.requests.get")
    def test_successful_request(self, mock_get):
        from xenon.utils.uw_api import uw_api_get
...
class TestIsMarketOpen:
    def test_weekday_during_hours(self):
        from xenon.utils.market_calendar import is_market_open
...
class TestGetLastNTradingDays:
    def test_returns_correct_count(self):
        from xenon.utils.market_calendar import get_last_n_trading_days
...
class TestClientIds:
    def test_registry_has_all_entries(self):
        from xenon.utils.ib_connection import CLIENT_IDS
...
class TestIbDefaults:
    def test_default_host(self):
        from xenon.utils.ib_connection import DEFAULT_HOST
    ...
    def test_default_gateway_port(self):
        # DEFAULT_GATEWAY_PORT now derives from XENON_TRADING_MODE — verify it
        # tracks the trading_mode contract rather than asserting a fixed value.
        from xenon.api.trading_mode import EXPECTED_PORT
        from xenon.utils.ib_connection import DEFAULT_GATEWAY_PORT

        assert DEFAULT_GATEWAY_PORT == EXPECTED_PORT

    def test_default_tws_port(self):
        from xenon.utils.ib_connection import DEFAULT_TWS_PORT

        assert DEFAULT_TWS_PORT == 7497


class TestConnectIb:
    @patch("xenon.utils.ib_connection.IB")
    def test_connects_with_registry_id(self, MockIB):
        from xenon.utils.ib_connection import connect_ib
        ...
        from xenon.utils.ib_connection import DEFAULT_GATEWAY_PORT as _DP
        from xenon.utils.ib_connection import DEFAULT_HOST as _DH

        mock_ib.connect.assert_called_once_with(_DH, _DP, clientId=0, timeout=10)
    ...
    def test_custom_host_and_port(self, MockIB):
        ...
        mock_ib.connect.assert_called_once_with("192.168.1.1", 7497, clientId=0, timeout=10)
    ...
    def test_custom_client_id_override(self, MockIB):
        ...
        mock_ib.connect.assert_called_once_with(_DH, _DP, clientId=999, timeout=10)
    ...
    def test_custom_timeout(self, MockIB):
        ...
        mock_ib.connect.assert_called_once_with(_DH, _DP, clientId=0, timeout=30)
