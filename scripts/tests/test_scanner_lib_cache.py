...
def test_read_cache_returns_data(tmp_path):
    from xenon.scanners._shared.cache import read_json_cache, write_json_cache
...
def test_read_cache_returns_none_for_missing(tmp_path):
    from xenon.scanners._shared.cache import read_json_cache
...
def test_read_cache_staleness(tmp_path):
    ...
    assert data is not None
    assert data["val"] == 1
    from xenon.scanners._shared.cache import is_stale
