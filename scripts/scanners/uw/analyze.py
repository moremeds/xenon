...
def _build_setup_thesis(td: TickerData, vrp: VRPState, regime: RegimeState, scores: BucketScores) -> dict:
    ...
    if regime.regime == "R2":
        family = "no_trade_R2"
        rationale = f"Regime is R2 ({regime.reason}); risk-off — no new structures."
    elif abs(composite) >= 40 and vrp.ts_inverted is True and iv_signal is not None and iv_signal > 70:
        family = "iron_condor"
        rationale = (
        ...
        family = "neutral"
        rationale = f"Mixed composite {composite:+.0f} without a clear vol or structural edge; stand aside."
    ...
    *,
    ...
    try:
        ...
        if scores.reweighted:
            notes.append(f"Buckets reweighted due to missing data: {scores.skipped_buckets}")
...
def _format_summary(report: AnalysisReport) -> str:
    ...
    lines.append(bar)
    lines.append(f"{report.ticker}  |  {s.bias}  ({s.composite:+.0f})  |  Grade {s.grade}")
    ...
    flip_dist = f"{abs(r.flip_distance_pct):.1f}%" if r.flip_distance_pct is not None else "—"
    gex_sign = r.gex_sign or "—"
    gpp = td.gamma_per_1pct if td else None
    lines.append(f"  GEX flip: {flip_val} ({flip_dist})  ·  Net γ sign: {gex_sign}  ·  γ per 1%: {_fmt(gpp, ',.0f')}")
    cw_strike = td.call_wall_strike if td else None
    pw_strike = td.put_wall_strike if td else None
    lines.append(f"  Call wall: {_fmt(cw_strike, '.2f')}  ·  Put wall: {_fmt(pw_strike, '.2f')}")
    ...
    lines.append(f"  Net premium  call: {_fmt(ncp, ',.0f')}  ·  put: {_fmt(npp, ',.0f')}  ·  net: {_fmt(nt, ',.0f')}")
    sv_ratio = td.short_volume_ratio if td else None
    sv_trend = td.short_volume_trend if td else None
    sv_trend_str = " → ".join(f"{t:.2f}" for t in sv_trend) if sv_trend else "—"
    lines.append(f"  Short vol ratio: {_fmt(sv_ratio, '.2f')}  ·  3-day trend: {sv_trend_str}")
    ...
    lines.append("  NOTE: No strikes/credits/stops — trade construction requires a chain fetch.")
