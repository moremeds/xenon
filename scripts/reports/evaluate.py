...
def fetch_seasonality(ticker: str) -> Dict:
    ...
    try:
        ...
        return {"rating": "UNKNOWN", "win_rate": None, "avg_return": None, "note": f"Fetch error: {e}"}
...
def determine_edge(
    ...
    result["passed"] = True
    if primary_pass:
        result["reason"] = f"{sustained} consecutive days of {agg_direction.lower()}, strength {agg_strength:.1f}"
    else:
        result["reason"] = (
            f"Recent strength {recent_strength:.1f} >70 (alternative criterion). Sustained days = {sustained}."
        )
...
def run_evaluations(
    ...
    results = [
        run_evaluation(t, bankroll=bankroll, price_history=price_cache.get(t, []), flow_days=flow_days) for t in tickers
    ]
    return results
...
def format_report(result: EvaluationResult) -> str:
    ...
    if m1d and m1d.data:
        ...
        if ns.get("total", 0) > 0:
            ...
            lines.append("-" * 60)
            lines.append(
                f"  Headlines:   {ns.get('total', 0)} "
                f"({ns.get('bullish', 0)} bull, "
                f"{ns.get('bearish', 0)} bear, "
                f"{ns.get('neutral', 0)} neutral)"
            )
            lines.append(
                f"  Sentiment:   {ns.get('sentiment_bias', 'N/A')} (score: {ns.get('avg_sentiment_score', 0):.2f})"
            )
...
def main():
    parser = argparse.ArgumentParser(description="Run full evaluation for one or more tickers.")
    parser.add_argument("tickers", nargs="+", help="Ticker symbol(s) to evaluate")
    parser.add_argument("--bankroll", type=float, default=1_200_000, help="Current bankroll (default: 1,200,000)")
    parser.add_argument("--days", type=int, default=5, help="Dark-pool lookback trading days (default: 5)")
    parser.add_argument("--json", action="store_true", help="Output raw JSON instead of formatted report")
    parser.add_argument(
        "--fast", action="store_true", help="Skip IB price history (faster, skips signal_priced_in check)"
    )
