...
def classify_trade(trade: Dict[str, Any]) -> str:
    ...
    if risk_profile == "UNDEFINED":
        ...
        has_short_put = any(str(l.get("type", "")).upper() in ("SHORT PUT", "SHORT_PUT") for l in legs)
        has_long_call = any(str(l.get("type", "")).upper() in ("LONG CALL", "LONG_CALL") for l in legs)
...
def _is_closed(trade: Dict[str, Any]) -> bool:
    ...
    return (pnl is not None and pnl != 0) or decision == "CLOSED" or "CLOSED" in action
...
def build_attribution(
    ...
    return {
        ...
        "by_strategy": [
            asdict(strategy_map[sid])
            for sid in sorted(strategy_map.keys(), key=lambda s: strategy_map[s].realized_pnl, reverse=True)
        ],
        "by_ticker": [asdict(t) for t in tickers_sorted],
        "by_edge": [
            asdict(edge_map[et]) for et in sorted(edge_map.keys(), key=lambda e: edge_map[e].realized_pnl, reverse=True)
        ],
        "by_risk": [
            asdict(risk_map[rt]) for rt in sorted(risk_map.keys(), key=lambda r: risk_map[r].realized_pnl, reverse=True)
        ],
