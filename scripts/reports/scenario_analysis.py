...
def approx_delta(spot, strike, dte, opt_type):
    ...
    sigma = 0.35
    d1 = (math.log(spot / strike) + (0.5 * sigma**2) * time_years) / (sigma * math.sqrt(time_years))
...
def scenario_price_shock(portfolio, spots, shock_pct):
    ...
    """
    positions = portfolio.get("positions") or []
    shocked_spots = {ticker: price * (1.0 + shock_pct) for ticker, price in spots.items()}
    position_impacts = []
    net_liq_change = 0.0
...
def scenario_delta_decay(portfolio, spots, decay_pct):
    ...
    for position in positions:
        ...
        for leg in legs:
            ...
            leg_delta = (
                sign * approx_delta(spot, strike, _option_dte(position.get("expiry")), opt_type) * contracts * 100.0
            )
...
def main():
    ...
    for scenario in ["bear", "base", "bull"]:
        ...
        for r in sorted(results[scenario], key=lambda x: x["pnl"]):
            ...
            print(
                f"  {r['ticker']:5s} {r['structure_type']:20s} | "
