...
describe("formatOrderError", () => {
  it("strips transport wrappers and rewrites IB margin rejection to concise operator copy", () => {
    const raw = "Xenon API 502: IB error 201: Order rejected - reason:YOUR ORDER IS NOT ACCEPTED. IN ORDER TO OBTAIN THE DESIRED POSITION YOUR PREVIOUS DAY EQUITY WITH LOAN VALUE <E> (644770.54 USD) MUST EXCEED THE INITIAL MARGIN (67243.00 USD).";
  ...
  it("collapses generic IB cancellation text to a readable sentence", () => {
    expect(formatOrderError("Xenon API 502: Order rejected by IB: Cancelled")).toEqual({
  ...
  it("keeps the legacy string formatter aligned with the structured formatter", () => {
    expect(formatOrderErrorMessage("Xenon API 502: Order rejected by IB: no acknowledgement (Unknown)")).toBe(
