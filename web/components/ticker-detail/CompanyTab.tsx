...
export default function CompanyTab({
  ...
  const fetchInfo = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/ticker/info?ticker=${encodeURIComponent(ticker)}`,
      );
      if (!res.ok) {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch company info",
      );
    } finally {
      setLoading(false);
      setFetched(true);
  ...
  if (loading) {
    return (
      <div className="tab-loading">
        <div className="tab-loading-text">Loading company info...</div>
      </div>
    );
  }
  if (error) {
  ...
  // ETF / index gating: indices and fund-like instruments have no equity
  // fundamentals (P/E, EPS, earnings). Hide those rows rather than show "---".
  ...
  // Market Cap is hidden only for fund/index types that have no cap value;
  // a present (non-zero) market cap still shows even for ETFs.
  const marketCapNum =
    marketCap == null
      ? null
      : typeof marketCap === "number"
        ? marketCap
        : parseFloat(String(marketCap));
  const hideMarketCap =
    hideEquityFundamentals &&
    (marketCapNum == null || marketCapNum === 0 || isNaN(marketCapNum));

  // Stat items — equity fundamentals (P/E, EPS, Next Earnings) are gated for
  // index/fund instruments; Div Yield is gated for indices only (ETFs/REITs can
  // pay distributions); Market Cap is gated only when absent on a fund type.
  type StatItem = { label: string; value: string };
  const statItems: StatItem[] = [
    !hideMarketCap && { label: "Market Cap", value: fmtMktCap(marketCap) },
    !hideEquityFundamentals && {
      label: "P/E Ratio",
      value:
        peRatio != null && !isNaN(Number(peRatio))
    ...
    !isIndexType && {
      label: "Div Yield",
      value:
        dividendYield != null && !isNaN(Number(dividendYield))
    ...
    {
      label: "High Today",
      value: todayHigh != null ? fmtNum(todayHigh) : "---",
    },
    ...
    {
      label: "52W High",
      value: week52High != null ? fmtNum(week52High) : "---",
    },
    { label: "52W Low", value: week52Low != null ? fmtNum(week52Low) : "---" },
    !hideEquityFundamentals && {
      label: "Next Earnings",
      value: nextEarnings != null ? String(nextEarnings) : "---",
    },
    ...
    <div className="company-tab">
      ...
      <div className="company-about">
        <div className="company-about-header">
          <span className="ratings-targets-title">About {ticker}</span>
          {(sector || issueType) && (
