...
| Flex polling fix         | `src/xenon/trade_blotter/flex_query.py`                     | Fixed `<FlexStatements` attribute match, `YYYYMMDD;HHMMSS` datetime, 120s timeout |
| NAV history tracking     | `src/xenon/execution/ib_sync.py` → `data/nav_history.jsonl` | Appends daily NAV on each portfolio sync                                          |
...
| Performance script       | `src/xenon/reports/portfolio_performance.py`                | NAV-based primary path + reconstruction fallback                                  |
