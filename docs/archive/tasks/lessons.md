...
- `NEXT_PUBLIC_XENON_API_URL` (browser env var) doesn't work for cross-origin ws-ticket in local dev (browser on :3000, FastAPI on :8321). Solution: route through Next.js API proxy (`/api/ib/ws-ticket`) which calls FastAPI server-to-server.
...
- For order-placement failures, never dump transport-layer strings like `Xenon API 502: ...` into the UI. Preserve upstream status/detail at the Next route boundary, then convert broker prose into short operator-facing summary/detail copy in a shared banner.
