...
```bash
xenon-evaluate [TICKER]        # human-readable
xenon-evaluate [TICKER] --json  # structured JSON
```
...
xenon-fetch-oi [TICKER]

# Filter for significant positions only
xenon-fetch-oi [TICKER] --min-premium 1000000

# Market-wide scan (for discover command)
xenon-fetch-oi --market --min-premium 10000000

# Verify specific external claims
xenon-verify-options-oi [TICKER] --expiry [DATE] --verify "strike1:size1,strike2:size2"
```
...
```bash
xenon-kelly --prob [P] --odds [ODDS] --bankroll [B]
```
...
1. Agent prompts: _"What is the change in the overall market?"_
...
4. Updates `src/xenon/reports/scenario_analysis.py` with scenario parameters
