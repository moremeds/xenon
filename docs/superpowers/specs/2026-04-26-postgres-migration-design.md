...
4. **Import UW data** — uw_analyze_cache.json → uw_snapshots; uw_unusual_flow_log.json → uw_flow_events; uw_api_stats_history.json → uw_api_stats
...
- **Partitioning** — `uw_snapshots` and `scan_results` by month if volume grows
